...
2. Elige `guiders-wp-plugin-2.14.4.zip`
...
= 2.14.4 =
* El ZIP ya no incluye el bundle de webpack-dev-server (HMR). Ese cliente recargaba la página cada segundo en producción.

= 2.14.3 =
* El asistente pide nombre, email, teléfono y comentarios (todos obligatorios) y solo la casilla de privacidad.
...
* Identify espera a tener el widget antes de reutilizar el chat abierto (evita hilo sin chatId).
...
= 2.13.3 =
* **🐛 Bug Fixes**:
  * **Detección de host correcta**: Ahora usa `window.location.host` en lugar de `hostname` para incluir el puerto, evitando problemas cuando el sitio usa un puerto no estándar.

= 2.13.2 =
* **🔧 Mejora del gesto de cierre en móvil**: El arrastre hacia abajo para cerrar el chat ahora sigue el dedo en tiempo real, con animación spring-back al soltar antes del umbral y deslizamiento fluido al cerrar. La pastilla indicadora es más sutil y discreta.
